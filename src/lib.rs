...
//!
//! Satoxid is a library to help encoding SAT problems with a focus on ergonomics and debugability.
//!
//! ## Example
//! ```rust
...
//!
//! While this is a simple API for a SAT solver, it can be difficult to use for the user 
//! to encode a problem.
...
//! }
//! ```
//! A value of `Tile` has the meaning that the tile at position xy has the number in 
//! value.
//!
...
//! When using functions like [`add_constraint_implies_repr`]() satoxid generates new 
//! variables which have no relation to the user defined SatVar like `Tile`.
...
//! A constraint is a type which can be turned in a finite amount of SAT clauses.
...
//! can only have exactly one value we would use the [`constraints::ExactlyK`] 
//! constraint.
//!
//! ```rust
...
//! ways. (e.g. We want that exactly atleast four different constraints are satisfied.)
...
//! Additionally it has a debug flag which enables/disables debug functionality of the
//! backend (printing the encoded constraints somewhere).
//!
...
//! user to call solve on the encoder.
...
/// Types which represents *every* used SAT variable by the encoder.
